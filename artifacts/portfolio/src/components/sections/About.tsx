...
export default function About() {
  return (
    <section
      id="about"
      className="relative z-10 overflow-hidden bg-[linear-gradient(180deg,hsl(var(--background))_0%,hsl(var(--background))_72%,rgba(16,185,129,0.035)_100%)] py-24"
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] gap-8 xl:gap-12 items-start">
          <FadeIn className="relative overflow-hidden rounded-[2rem] border border-border bg-white/70 backdrop-blur-sm p-8 sm:p-10 lg:p-12 shadow-[0_25px_70px_-45px_rgba(0,0,0,0.35)]">
            <div className="absolute inset-0 opacity-60 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.12),transparent_40%),radial-gradient(circle_at_bottom_right,rgba(16,185,129,0.05),transparent_45%)]" />
            <div className="relative">
              <span className="font-mono text-xs sm:text-sm uppercase tracking-[0.22em] text-primary font-bold">
                About Me
              </span>
              <div className="mt-4 grid gap-6 lg:grid-cols-[minmax(0,1fr)_220px] lg:gap-8 items-start">
                <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black leading-[0.95] text-foreground">
                  Building Unreal systems that still hold up in production.
                </h2>
                <p className="text-base sm:text-lg leading-relaxed text-muted-foreground lg:pt-2">
              ...
              <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-3">
                {proof.map((item) => (
                  <div
                    key={item.label}
                    className="rounded-2xl border border-border bg-background/90 px-4 py-4"
                  >
          ...
          <div className="space-y-4 lg:pt-6">
            ...
            {capabilities.map((item, index) => (
              <FadeIn
                ...
                className="group rounded-[1.75rem] border border-border bg-white/90 p-6 sm:p-7 shadow-[0_16px_34px_-28px_rgba(15,23,42,0.12)] transition-all duration-300 hover:-translate-y-1 hover:border-primary/25 hover:shadow-[0_20px_40px_-30px_rgba(15,23,42,0.16)]"
              >
